...
interface DiaMenu {
  ...
  platoUnico: any | null;
  cena: any | null;
  bloqueado?: boolean; // NUEVO: Estado para fijar el día
}
...
export default function App() {
  ...
  // NUEVO: Generación inteligente respetando los días bloqueados (candado)
  function generarMenuEstructurado(lista: any[]) {
    ...
    // Mantener los días que ya estaban bloqueados y registrar sus IDs como usados
    const menuActualMap: { [dia: string]: DiaMenu } = {};
    menuSemanal.forEach(d => {
      if (d.bloqueado) {
        menuActualMap[d.dia] = d;
        if (d.esUnico && d.platoUnico) usadosEnLaSemana.add(d.platoUnico.id);
        if (!d.esUnico) {
          if (d.primero) usadosEnLaSemana.add(d.primero.id);
          if (d.segundo) usadosEnLaSemana.add(d.segundo.id);
        }
        if (d.cena) usadosEnLaSemana.add(d.cena.id);
      }
    });
    ...
    const nuevoMenu: DiaMenu[] = DIAS_SEMANA.map(dia => {
      if (menuActualMap[dia]) {
        return menuActualMap[dia]; // Si está bloqueado, se queda tal cual
      }

      const comensalesAnteriores = menuSemanal.find(d => d.dia === dia)?.comensales || 1;
      const esUnico = platosUnicos.some(r => !usadosEnLaSemana.has(r.id)) && Math.random() < 0.5;
      
      return {
        dia,
        comensales: comensalesAnteriores,
        esUnico,
  ...
  function toggleBloquearDia(diaNombre: string) {
    setMenuSemanal(prev => prev.map(d => d.dia === diaNombre ? { ...d, bloqueado: !d.bloqueado } : d));
  }
        ...
        <div style={{ width: '100%', maxWidth: '420px', backgroundColor: '#FFFFFF', color: '#2C2A29', padding: '40px', borderRadius: '24px', boxShadow: '0 10px 25px -5px rgba(0, 0, 0, 0.05), 0 8px 10px -6px rgba(0, 0, 0, 0.05)', border: '1px solid #E6DFD3' }}>
          ...
          <form onSubmit={handleAuth} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div>
              ...
              <input type="email" required placeholder="tu@correo.com" value={authEmail} onChange={e => setAuthEmail(e.target.value)} style={{ width: '100%', padding: '12px 16px', borderRadius: '12px', border: '1px solid #D6D3D1', fontSize: '14px', outline: 'none', boxSizing: 'border-box', backgroundColor: '#FAFAF9', color: '#2C2A29', transition: 'all 0.2s' }} />
            </div>
            <div>
              <label style={{ fontSize: '13px', fontWeight: '600', color: '#44403C', display: 'block', marginBottom: '6px' }}>Contraseña</label>
              <input type="password" required placeholder="••••••••" value={authPassword} onChange={e => setAuthPassword(e.target.value)} style={{ width: '100%', padding: '12px 16px', borderRadius: '12px', border: '1px solid #D6D3D1', fontSize: '14px', outline: 'none', boxSizing: 'border-box', backgroundColor: '#FAFAF9', color: '#2C2A29', transition: 'all 0.2s' }} />
            </div>
            ...
            <div style={{ backgroundColor: '#FFFFFF', borderRadius: '20px', padding: '32px 24px', border: '1px solid #E6DFD3', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.02)' }}>
              ...
              <div style={{ backgroundColor: '#F9F8F6', padding: '16px', borderRadius: '12px', border: '1px solid #E7E5E4' }}>
                ...
                <ul style={{ margin: 0, paddingLeft: '18px', fontSize: '13px', color: '#57534E', lineHeight: '1.5', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                  <li>Generación inteligente de menús semanales con opción de bloquear días concretos.</li>
                      ...
                      <div key={item.dia} style={{ perspective: '1000px', width: '100%' }}>
                        <div style={{
                          ...
                          <div style={{
                            ...
                            border: estaBloqueado ? '2px solid #D97706' : '1px solid #E6DFD3',
                            borderRadius: '20px',
                            padding: '20px',
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid #F5F2EB', paddingBottom: '12px', marginBottom: '14px', flexWrap: 'wrap', gap: '10px' }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                ...
                                {/* NUEVO: Botón de candado para fijar/bloquear el día */}
                                <button 
                                  onClick={() => toggleBloquearDia(item.dia)} 
                                  title={estaBloqueado ? "Desbloquear día para que cambie al regenerar" : "Bloquear día para mantenerlo al regenerar"}
                                  style={{
                                    background: estaBloqueado ? '#FEF3C7' : '#F5F2EB',
                                    border: estaBloqueado ? '1px solid #F59E0B' : '1px solid #E7E5E4',
                                    borderRadius: '6px',
                                    padding: '2px 6px',
                                    fontSize: '11px',
                                    cursor: 'pointer',
                                    ...
                                    alignItems: 'center',
                                    gap: '4px',
                                    color: estaBloqueado ? '#B45309' : '#78716C',
                                    fontWeight: '600'
                                  }}
                                >
                                  {estaBloqueado ? '🔒 Fijado' : '🔓 Fijo'}
                                </button>
                              </div>
